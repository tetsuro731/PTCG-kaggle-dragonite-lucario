...
# --- learned action scorer ---------------------------------------------------
# CatBoost is exported as a pure-Python tree evaluator. Any import, feature,
# or scoring failure falls back to the rule policy below.
if "__file__" in globals():
...
try:
    if __package__:
        ...
        from . import features_window as _features
        from . import model_data as _model
    else:
        import damage as _damage
        import features_window as _features
        import model_data as _model
...
_feature_state = (
    _features.new_episode_state()
    if _features is not None and hasattr(_features, "new_episode_state")
    else None
)


# This is narrowed after per-context validation. Until a model is published,
# every context automatically falls back to the rule scorer.
MODEL_CONTEXTS = {
    SelectContext.MAIN,
...
class LucarioPolicy:
    ...
    def choose(self) -> list[int]:
        ...
        self._suppress_useless_attacks(scores)
        ranked = [
            i
            for i, _ in sorted(
        ...
        if _feature_state is not None:
            _features.update_episode_state(_feature_state, self.obs, chosen)
        return chosen

    def _suppress_useless_attacks(self, scores: list[float]) -> None:
        if _damage is None or not self.me.active or not self.opponent.active:
            return
        attacker = self.me.active[0]
        defender = self.opponent.active[0]
        if attacker is None or defender is None:
            return
        for index, option in enumerate(self.select.option):
            if option.type != OptionType.ATTACK:
                continue
            result = _damage.calculate_attack_damage(
                attacker,
                defender,
                option.attackId,
                self.stadium_id,
                _feature_state,
                defender_player_index=self.op_index,
            )
            aura_jab_can_accelerate = (
                option.attackId == AURA_JAB
                and self.discard_counts[C.BASIC_FIGHTING_ENERGY] > 0
            )
            if result.final <= 0 and not aura_jab_can_accelerate:
                scores[index] = float("-inf")

    def _model_scores(self) -> list[float] | None:
        """Return a learned score per option, or fall back to rule scores."""
        if _model is None or _features is None:
            return None
        if self.context not in MODEL_CONTEXTS or len(self.select.option) < 2:
            return None
        try:
            builder = (
                _features.Features(self.obs, my_deck)
                if _feature_state is None
                else _features.build_features(self.obs, my_deck, _feature_state)
            )
            state = builder.state
            model_feature_names = getattr(_model, "FEATURE_NAMES", None)
            feature_indices = getattr(_model, "FEATURE_INDICES", None)
            scores = []
            for index in range(len(self.select.option)):
                row = state + builder.option(index)
                if model_feature_names is not None:
                    values = dict(zip(_features.FEATURE_NAMES, row))
                    row = [values[name] for name in model_feature_names]
                elif feature_indices is not None:
                    row = [row[feature_index] for feature_index in feature_indices]
                scores.append(_model.score(row))
            return scores
        except Exception:  # noqa: BLE001 - never lose a game to a model error
            return None
